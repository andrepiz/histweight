function [M, I] = permn(V, N, K) %#codegen
 % DEVNOTE: may be a pain in the ass to codegen TBC. Modified for compatibility (TO TEST)
arguments
    V  (:,:) double
    N  (:,:) double = 0
    K  (:,:) double = 0
...
narginchk(2, 3) ;
assert(not(fix(N) ~= N || N < 0 || numel(N) ~= 1), 'permn:negativeN','Second argument should be a positive integer');
...
if nargin == 2 
    ...
    elseif N == 1
        ...
        Y = cell(1, N);
        [Y{N:-1:1}] = ndgrid(1:nV) ; % DEVNOTE: Y is NOT DEFINED IN ALL EXECUTION PATHS!
    ...
    assert(not( nK < 1 || any(K<1) || any(K ~= fix(K)) ), 'permn:InvalidIndex','Third argument should contain positive integers.');
    ...
    if nV == 0 || N == 0 || nK == 0
        ...
        if any(K > Npos)
            fprintf('permn:IndexOverflow:, Values of K exceeding the total number of combinations are saturated.')
            K = min(K, Npos) ;
        end
